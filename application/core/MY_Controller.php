...
class MY_Controller extends MX_Controller {
	...
	protected $mCurrency_symbol = '£';
	protected $mCurrency_rate = '1';
	...
	public function __construct()
	{
		...
		$this->mViewData['current_user_obj']=array();
		//$this->mViewData['current_user_ordercnt']=0;
		if($this->session->has_userdata('current_user')){
			$this->mViewData['current_user']=$this->session->userdata('current_user');
			$this->mViewData['current_user_obj']=$this->session->userdata('current_user_obj');
			/*
			$this->load->model('users_model', 'users');
			$this->mViewData['current_user_ordercnt'] = count($this->users->execute_query("select a.* from products_custom a left join addresses b on a._addressid=b.ids where a._user='{$this->mViewData['current_user']}'"));
			*/
		}

		$config = $this->config->item('ci_bootstrap');
		$lang_config = empty($config['languages']) ? array() : $config['languages'];
		if(isset($lang_config['available'])){
